...
module "sg" { // overrides variables set in /modules/aws_security_group/tcp/variables.tf directory
  ...
  protocol     = "tcp"
  allow_ports  = ["80", "443"]
  ingress_cidr = "0.0.0.0/0" //for internet access
...
module "sg1" { // overrides variables set in /modules/aws_security_group/tcp/variables.tf directory
  ...
  protocol     = "tcp"
  allow_ports  = ["22"]
  ingress_cidr = data.terraform_remote_state.network.outputs.vpc_cidr // access from internal
  vpc_id       = data.terraform_remote_state.network.outputs.vpc_id
}

module "sg2" { // overrides variables set in /modules/aws_security_group/tcp/variables.tf directory
  source = "../../../modules/aws_security_group/icmp-ping/"
  //source       = "git@github.com:avvppro/terraform.git//modules/aws_security_group/icmp-ping"
  env          = "dev"
  name         = "sg2"
  ingress_cidr = "0.0.0.0/0" //for internet access
  vpc_id       = data.terraform_remote_state.network.outputs.vpc_id
